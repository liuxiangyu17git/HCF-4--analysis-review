#!/usr/bin/env Rscript
# ============================================================================
# 脚本: 03_variable_basic_L.R
# 描述: NHANES 2021-2023 (L周期) 基础变量构建 - 阶段一
# 
...
# 启动日志记录（期刊要求）
log_file <- file.path(LOG_DIR, paste0("03_variable_basic_L_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".log"))
sink(log_file, split = TRUE)
cat("========================================================\n")
cat("脚本: 03_variable_basic_L.R\n")
cat("描述: NHANES 2021-2023 (L周期) 基础变量构建\n")
...
data <- readRDS(file.path(clean_dir, "master.rds"))
...
# ============================================================================
# 5. behavior_vars.rds（行为学变量）- 完全遵循详表
# ============================================================================
cat("\n4.1 构建behavior_vars.rds...\n")
...
behavior_data$wtmec2yr <- ifelse(!is.na(data$WTMEC2YR) & data$WTMEC2YR > 0, data$WTMEC2YR, NA)
saveRDS(behavior_data, file.path(clean_dir, "behavior_vars.rds"))
cat(sprintf(" ✅ 已保存: behavior_vars.rds (%d个变量)\n", ncol(behavior_data)))
# ============================================================================
# 6. clinical_vars.rds（临床诊断变量）- 完全遵循详表
# ============================================================================
cat("\n4.2 构建clinical_vars.rds...\n")
...
for(i in 1:nrow(data)) {
  if(data$is_adult[i] == 1) {
    ...
    if(!is.na(data$BPQ150[i]) && data$BPQ150[i] == 1) ht <- 1
...
clinical_data$wtmec2yr <- behavior_data$wtmec2yr
saveRDS(clinical_data, file.path(clean_dir, "clinical_vars.rds"))
cat(sprintf(" ✅ 已保存: clinical_vars.rds (%d个变量)\n", ncol(clinical_data)))
# ============================================================================
# 7. social_vars.rds（社会决定因素）
# ============================================================================
cat("\n4.3 构建social_vars.rds...\n")
...
if("OCD150" %in% names(data)) {
  for(i in 1:nrow(data)) {
    if(data$is_adult[i] == 1 && !is.na(data$OCD150[i])) {
      if(data$OCD150[i] == 1) social_data$employment_status[i] <- "就业"
      else if(data$OCD150[i] == 2) social_data$employment_status[i] <- "在职休假"
      else if(data$OCD150[i] == 3) social_data$employment_status[i] <- "失业"
      else if(data$OCD150[i] == 4) social_data$employment_status[i] <- "不工作"
      social_data$employed[i] <- ifelse(data$OCD150[i] == 1, 1, 0)
    }
  }
}
...
if("WTINT2YR" %in% names(data)) {
  social_data$wtint2yr <- data$WTINT2YR
} else {
  social_data$wtint2yr <- NA
  warning("WTINT2YR不存在于原始数据中")
}
saveRDS(social_data, file.path(clean_dir, "social_vars.rds"))
cat(sprintf(" ✅ 已保存: social_vars.rds (%d个变量)\n", ncol(social_data)))
# ============================================================================
# 8. symptom_vars.rds（症状与功能）
# ============================================================================
cat("\n4.4 构建symptom_vars.rds...\n")
...
if("WTINT2YR" %in% names(data)) {
  symptom_data$wtint2yr <- data$WTINT2YR
} else {
  symptom_data$wtint2yr <- NA
}
saveRDS(symptom_data, file.path(clean_dir, "symptom_vars.rds"))
cat(sprintf(" ✅ 已保存: symptom_vars.rds (%d个变量)\n", ncol(symptom_data)))
# ============================================================================
# 9. dietbehavior_vars.rds（饮食行为变量）
# ============================================================================
cat("\n4.5 构建dietbehavior_vars.rds...\n")
diet_data <- data.frame(SEQN = data$SEQN)
# 核心饮食行为变量
diet_data$diet_special <- NA
if("DRQSDIET" %in% names(data)) {
  for(i in 1:nrow(data)) {
    if(data$is_adult[i] == 1 && !is.na(data$DRQSDIET[i])) {
      diet_data$diet_special[i] <- ifelse(data$DRQSDIET[i] == 1, 1, 0)
    }
  }
}
# 减肥饮食
...
saveRDS(diet_data, file.path(clean_dir, "dietbehavior_vars.rds"))
cat(sprintf(" ✅ 已保存: dietbehavior_vars.rds (%d个变量)\n", ncol(diet_data)))
# ============================================================================
# 10. healthbehavior_vars.rds（健康行为达标变量）
# ============================================================================
cat("\n4.6 构建healthbehavior_vars.rds...\n")
...
# 体力活动变量转换函数
convert_to_weekly <- function(freq, unit) {
  if(is.na(freq) || is.na(unit)) return(NA)
  if(unit == "D") return(freq * 7)
  if(unit == "W") return(freq)
  if(unit == "M") return(freq / 4.33)
  if(unit == "Y") return(freq / 52.14)
  return(NA)
}
# 初始化变量
...
# 检查必要变量
has_moderate <- all(c("PAD790Q", "PAD790U", "PAD800") %in% names(data))
has_vigorous <- all(c("PAD810Q", "PAD810U", "PAD820") %in% names(data))
if(has_moderate || has_vigorous) {
  cat("\n 🔧 构建体力活动变量...\n")
  for(i in 1:nrow(data)) {
    if(!is.na(data$is_adult[i]) && data$is_adult[i] == 1) {
      ...
      # 中等强度
      if(has_moderate) {
        mod_freq <- data$PAD790Q[i]
        mod_unit <- data$PAD790U[i]
        mod_mins <- data$PAD800[i]
        if(!is.na(mod_freq) && !is.na(mod_unit) && !is.na(mod_mins)) {
          if(mod_freq == 0) {
            total_min <- total_min + 0
            has_activity <- TRUE
          } else if(mod_freq > 0 && mod_mins > 0 && mod_mins <= 720) {
            mod_weekly <- convert_to_weekly(mod_freq, mod_unit)
            if(!is.na(mod_weekly)) {
              total_min <- total_min + (mod_weekly * mod_mins)
              has_activity <- TRUE
            }
          }
        }
      ...
      # 剧烈强度
      if(has_vigorous) {
        vig_freq <- data$PAD810Q[i]
        vig_unit <- data$PAD810U[i]
        vig_mins <- data$PAD820[i]
        if(!is.na(vig_freq) && !is.na(vig_unit) && !is.na(vig_mins)) {
          if(vig_freq == 0) {
            total_min <- total_min + 0
            has_activity <- TRUE
          } else if(vig_freq > 0 && vig_mins > 0 && vig_mins <= 900) {
            vig_weekly <- convert_to_weekly(vig_freq, vig_unit)
            if(!is.na(vig_weekly)) {
              total_min <- total_min + (vig_weekly * vig_mins * 2)
              has_activity <- TRUE
            }
          }
        }
...
health_data$wtmec2yr <- ifelse(!is.na(data$WTMEC2YR) & data$WTMEC2YR > 0, data$WTMEC2YR, NA)
if("WTINT2YR" %in% names(data)) {
  health_data$wtint2yr <- ifelse(!is.na(data$WTINT2YR) & data$WTINT2YR > 0, data$WTINT2YR, NA)
} else {
  health_data$wtint2yr <- NA
}
saveRDS(health_data, file.path(clean_dir, "healthbehavior_vars.rds"))
cat(sprintf("\n ✅ 已保存: healthbehavior_vars.rds (%d个变量)\n", ncol(health_data)))
# ============================================================================
...
behavior_cols <- intersect(
  c("SEQN", "is_adult", "is_non_pregnant_adult", "current_smoker",
    ...
    "binge_drinker", "sdmvpsu", "sdmvstra", "wtint2yr"),
  names(behavior_data)
)
...
clinical_cols <- intersect(
  c("SEQN", "bmi_continuous", "bmi_category", "obesity",
    ...
    "crp", "high_crp", "crp_category", "wtmec2yr", "sdmvpsu", "sdmvstra"),
  names(clinical_data)
)
...
social_cols <- intersect(
  c("SEQN", "age", "gender", "race_ethnicity", "education_level",
    ...
    "below_poverty", "wtint2yr", "sdmvpsu", "sdmvstra"),
  names(social_data)
)
...
symptom_cols <- intersect(
  c("SEQN", "self_rated_health", "poor_self_rated_health",
    "phq9_total", "depression", "wtint2yr", "sdmvpsu", "sdmvstra"),
  names(symptom_data)
)
...
health_cols <- intersect(
  c("SEQN", "sleep_adequate", "pa_total_min_week",
    "pa_meets_guideline", "wtmec2yr", "wtint2yr", "sdmvpsu", "sdmvstra"),
  names(health_data)
)
...
if(nrow(interview_valid) > 0) {
  ...
  saveRDS(design_objects$interview, file.path(clean_dir, "interview_design.rds"))
  cat(" ✅ 已保存: interview_design.rds\n")
}
# MEC权重设计对象
cat("\n 5.2.2 创建MEC权重设计对象 (WTMEC2YR)...\n")
...
if(nrow(mec_valid) > 0) {
  ...
  saveRDS(design_objects$mec, file.path(clean_dir, "mec_design.rds"))
  cat(" ✅ 已保存: mec_design.rds\n")
}
# 膳食权重设计对象
...
if(nrow(diet_valid) > 0) {
  ...
  saveRDS(design_objects$diet, file.path(clean_dir, "diet_design.rds"))
  cat(" ✅ 已保存: diet_design.rds\n")
}
...
if(!is.null(design_objects$interview)) {
  if("is_adult" %in% names(design_objects$interview$variables)) {
    ...
    saveRDS(adult_interview, file.path(clean_dir, "adult_interview_design.rds"))
    cat(" ✅ 已保存: adult_interview_design.rds\n")
  }
}
if(!is.null(design_objects$mec)) {
  if("is_adult" %in% names(design_objects$mec$variables)) {
    ...
    saveRDS(adult_mec, file.path(clean_dir, "adult_mec_design.rds"))
    cat(" ✅ 已保存: adult_mec_design.rds\n")
  }
  if("is_non_pregnant_adult" %in% names(design_objects$mec$variables)) {
    ...
    saveRDS(non_preg_mec, file.path(clean_dir, "non_pregnant_adult_mec_design.rds"))
    cat(" ✅ 已保存: non_pregnant_adult_mec_design.rds\n")
  }
}
if(!is.null(design_objects$diet)) {
  if("is_adult" %in% names(design_objects$diet$variables)) {
    ...
    saveRDS(adult_diet, file.path(clean_dir, "adult_diet_design.rds"))
    cat(" ✅ 已保存: adult_diet_design.rds\n")
  }
}
cat("\n ✅ 5.3完成：子组创建完成\n")
# 创建设计对象索引
...
design_index <- list(
  interview = list(
    full = "interview_design.rds",
    adult = "adult_interview_design.rds"
  ),
  mec = list(
    full = "mec_design.rds",
    adult = "adult_mec_design.rds",
    non_pregnant = "non_pregnant_adult_mec_design.rds"
  ),
  diet = list(
    full = "diet_design.rds",
    adult = "adult_diet_design.rds"
  )
)
saveRDS(design_index, file.path(clean_dir, "design_index.rds"))
cat(" ✅ 已保存: design_index.rds\n")
# ============================================================================
# 12. 生成合规报告
# ============================================================================
cat("\n6. 生成NHANES教程合规报告...\n")
report_file <- file.path(LOG_DIR, "03_variable_basic_report_L.txt")
sink(report_file)
cat("NHANES基础变量构建 - 完全合规报告\n")
cat("========================================\n\n")
...
cat("二、变量构建完成清单\n")
cat(sprintf(" 1. behavior_vars.rds: %d个变量\n", ncol(behavior_data)))
cat(sprintf(" 2. clinical_vars.rds: %d个变量\n", ncol(clinical_data)))
cat(sprintf(" 3. social_vars.rds: %d个变量\n", ncol(social_data)))
cat(sprintf(" 4. symptom_vars.rds: %d个变量\n", ncol(symptom_data)))
cat(sprintf(" 5. dietbehavior_vars.rds: %d个变量\n", ncol(diet_data)))
cat(sprintf(" 6. healthbehavior_vars.rds: %d个变量\n", ncol(health_data)))
sink()
cat(" ✅ 合规报告已保存\n")
# ============================================================================
# 13. 保存会话信息（期刊要求）
# ============================================================================
cat("\n7. 保存会话信息...\n")
session_info_path <- file.path(LOG_DIR, "03_session_info_L.txt")
sink(session_info_path)
cat("NHANES基础变量构建会话信息\n")
cat("=========================\n")
...
code_save_path <- file.path(scripts_dir, "03_variable_basic_L.R")
cat("\n⚠️  请手动将当前脚本保存到以下位置：\n")
cat(sprintf("   %s\n\n", code_save_path))
cat("   这是JAMA Psychiatry的明确要求：所有分析代码必须保存并公开。\n")
code_list_path <- file.path(LOG_DIR, "03_code_list_L.txt")
cat("脚本名称: 03_variable_basic_L.R\n", file = code_list_path)
...
sink()
# 清理临时变量
rm(list = setdiff(ls(), c("clean_dir", "LOG_DIR", "design_objects")))
gc()
