...
cat("描述: NHANES 2021-2023 (L周期) 数据合并\n")
...
cat("\n")
# ============================================================================
# 2. 教程要求的文件加载顺序（完全遵循变量详表）
# ============================================================================
cat("步骤1: 验证清洗文件\n")
cat("----------------------------------------\n")
# 教程建议的合并顺序（完全遵循详表第一部分）
MERGE_ORDER <- c(
  "DEMO_L_clean.rds",   # 1. 人口统计 (必需第一个)
  "DPQ_L_clean.rds",    # 2. 抑郁问卷
  "SLQ_L_clean.rds",    # 3. 睡眠问卷
  "PAQ_L_clean.rds",    # 4. 体力活动问卷
  "DBQ_L_clean.rds",    # 5. 饮食行为问卷
  "DR1TOT_L_clean.rds", # 6. 24小时膳食回顾
  "WHQ_L_clean.rds",    # 7. 体重历史问卷
  "BMX_L_clean.rds",    # 8. 身体测量
  "BPXO_L_clean.rds",   # 9. 血压测量
  "BPQ_L_clean.rds",    # 10. 血压问卷
  "HSCRP_L_clean.rds",  # 11. C反应蛋白
  "CBC_L_clean.rds",    # 12. 血常规
  "INQ_L_clean.rds",    # 13. 收入贫困比
  "ALQ_L_clean.rds",    # 14. 酒精使用问卷
  "HUQ_L_clean.rds",    # 15. 健康问卷
  "SMQ_L_clean.rds",    # 16. 吸烟问卷
  "OCQ_L_clean.rds",    # 17. 职业问卷
  "BIOPRO_L_clean.rds", # 18. 生化指标
  "DIQ_L_clean.rds",    # 19. 糖尿病问卷
  "MCQ_L_clean.rds"     # 20. 医疗状况问卷
)
# 获取实际文件
actual_files <- list.files(CLEANED_PATH, pattern = "_clean\\.rds$")
...
# 检查DEMO是否存在（绝对必需）
if (!"DEMO_L_clean.rds" %in% files_to_merge) {
  stop("❌ 致命错误：DEMO_L_clean.rds不存在！这是教程要求的核心文件。")
}
cat("\n")
# 初始化合并日志
merge_log <- list(
...
load_and_validate <- function(file_name, step) {
  ...
  file_path <- file.path(CLEANED_PATH, file_name)
  file_log <- list(
    file = file_name,
    timestamp = Sys.time(),
    issues = list()
  ...
  if (file_name == "DEMO_L_clean.rds" && unique_seqn != nrow(data)) {
...
validation_results$sample_size <- list(
  expected = if ("DEMO_L_clean.rds" %in% files_to_merge) {
    demo <- readRDS(file.path(CLEANED_PATH, "DEMO_L_clean.rds"))
    nrow(demo)
  } else NA,
  actual = nrow(master_data),
  status = if ("DEMO_L_clean.rds" %in% files_to_merge) {
    demo <- readRDS(file.path(CLEANED_PATH, "DEMO_L_clean.rds"))
...
cat(" ✅\n")
# 保存数据集描述
desc_path <- file.path(CLEANED_PATH, "master_description.txt")
sink(desc_path)
cat("NHANES主数据集描述\n")
cat("===================\n\n")
...
code_save_path <- file.path(scripts_dir, "02_data_merge_L.R")
cat("\n⚠️  请手动将当前脚本保存到以下位置：\n")
cat(sprintf("   %s\n\n", code_save_path))
