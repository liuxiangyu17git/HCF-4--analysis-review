...
cat("描述: NHANES 2017-2020 (P周期) 论文3 - α因子正式分析（整合版）\n")
...
cat(sprintf("已加载: analysis_dataset_subset_P.rds\n"))
cat(sprintf("样本量: %d\n", nrow(data_full)))
cat(sprintf("变量数: %d\n\n", ncol(data_full)))
data <- data_full
cat(sprintf("分析样本: %d\n\n", nrow(data)))
...
if(length(alpha_vars) == 0) {
  stop("错误: 没有找到α因子变量，请先运行05_alpha_factors_P.R")
}
...
# 注释掉长格式保存，只保留宽格式作为正式的 Table 3
# write.csv(alpha_disease_results, file.path(RESULTS_DIR, "Table3_long_P.csv"), row.names = FALSE)
# 获取痴固着型α因子均值
cat("\n获取痴固着型α因子均值...\n")
# 初始化痴固着型均值数据框，使用与宽格式相同的列名
...
if(nrow(alpha_disease_results) > 0) {
  ...
  # 保存完整的Table 3 - 统一命名为 Table3_P.csv
...
alpha_pathway_file <- file.path(paper2_results_dir, "Table2_P.csv")
if(file.exists(alpha_pathway_file)) {
  alpha_pathway_data <- read.csv(alpha_pathway_file)
  cat("\n=== 数据检查 ===\n")
  cat("数据维度:", dim(alpha_pathway_data), "\n")
  cat("列名:", paste(names(alpha_pathway_data), collapse = ", "), "\n")
  # 转换为长格式
  alpha_pathway_long <- alpha_pathway_data %>%
    pivot_longer(cols = c(alpha1, alpha2, alpha3, alpha4),
    ...
    mutate(
      ...
      pathway_cluster_en = factor(pathway_cluster_en,
  ...
  p_alpha_pathway <- ggplot(alpha_pathway_long,
                            aes(x = pathway_cluster_en, y = value, fill = alpha_label)) +
    geom_bar(stat = "identity", position = "dodge", width = 0.7) +
    scale_fill_brewer(palette = "Set2") +
    labs(title = "Figure 2. Alpha Factor Distribution Across Pathways (P Cycle)",
  ...
  cat("❌ P周期文件不存在:", alpha_pathway_file, "\n")
}
# ============================================================================
...
code_save_path <- file.path(scripts_dir, "16_paper3_alpha_analysis_P.R")
cat("\n⚠️  请手动将当前脚本保存到以下位置：\n")
cat(sprintf("   %s\n\n", code_save_path))
...
# ============================================================================
cat("\n========================================================\n")
cat("✅ Paper 3 alpha factor analysis complete (P Cycle)!\n")
cat("Completion time:", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n")
