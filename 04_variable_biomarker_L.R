...
cat("描述: NHANES 2021-2023 (L周期) 生物标志物构建\n")
...
# ============================================================================
cat("1. 加载主数据集并验证完整性...\n")
master_file <- file.path(clean_dir, "master.rds")
if(!file.exists(master_file)) {
  stop("❌ 错误：数据文件不存在。请确认文件路径和名称。")
}
...
cat("变量数：", ncol(master), "\n")
# 检查关键必需变量
essential_vars <- c("SEQN", "SDMVPSU", "SDMVSTRA", "WTMEC2YR",
...
# 4. 构建生物标志物变量（不使用空腹数据）
# ============================================================================
cat("3. 构建生物标志物变量（不使用空腹数据）...\n")
# 4.1 肝功能指标
cat(" 3.1 肝功能指标...\n")
if("LBXSASSI" %in% names(data)) {
  data <- data %>%
    mutate(
      ast_ui_l = case_when(
...
if(!is.null(alt_source)) {
  data <- data %>%
    mutate(
      alt_ui_l = case_when(
        !!sym(alt_source) %in% c(7777, 777, 77, 9999, 999, 99, 77777, 99999) ~ NA_real_,
        !!sym(alt_source) < 0 ~ NA_real_,
        TRUE ~ as.numeric(!!sym(alt_source))
      )
    )
} else {
...
if(all(c("LBXNEPCT", "LBXLYPCT") %in% names(data))) {
  data <- data %>%
    mutate(
...
if("LBXWBCSI" %in% names(data)) {
  ...
  data$white_blood_cells <- NA_real_
}
data <- data %>%
  mutate(
    elevated_ast = case_when(
...
# 保存文件
saveRDS(analysis_data_full, file.path(clean_dir, "02_biomarker_full.rds"))
saveRDS(analysis_data_core, file.path(clean_dir, "02_biomarker_core.rds"))
cat(sprintf(" ✅ 完整版已保存: 02_biomarker_full.rds (%d个变量)\n", ncol(analysis_data_full)))
cat(sprintf(" ✅ 核心版已保存: 02_biomarker_core.rds (%d个变量)\n", ncol(analysis_data_core)))
# 生成版本说明
version_note <- data.frame(
  输出文件 = c("02_biomarker_full.rds", "02_biomarker_core.rds"),
...
write.csv(version_note, file.path(clean_dir, "02_biomarker_version_control.csv"), row.names = FALSE)
cat(" ✅ 版本说明已保存: 02_biomarker_version_control.csv\n\n")
# ============================================================================
# 6. 数据质量评估
# ============================================================================
cat("5. 数据质量评估...\n")
...
# ============================================================================
cat("6. 生成投稿材料...\n")
methods_file <- file.path(clean_dir, "methods_description_nhanes_no_fasting.txt")
sink(methods_file)
cat("NHANES数据分析方法（不使用空腹数据版本）\n")
cat("=============================================\n\n")
cat("数据来源\n")
cat("--------\n")
cat("本研究使用美国国家健康与营养调查（NHANES）2021-2023周期数据。\n\n")
cat("样本选择\n")
cat("--------\n")
...
code_save_path <- file.path(scripts_dir, "04_variable_biomarker_L.R")
cat("\n⚠️  请手动将当前脚本保存到以下位置：\n")
cat(sprintf("   %s\n\n", code_save_path))
