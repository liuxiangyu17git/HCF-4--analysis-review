...
cat("描述: NHANES 2017-2020 (P周期) 论文2补充分析\n")
...
# 检查必要变量
required_vars <- c("SDMVPSU", "SDMVSTRA", "WTINTPRP", "WTMECPRP")
...
if ("HCF_type" %in% names(high_risk_data) &&
  ...
  write.csv(cross_tab_df, file.path(RESULTS_DIR, "table_S1_cross_tab_counts_P.csv"))
  ...
  write.csv(prop_df, file.path(RESULTS_DIR, "table_S1_cross_tab_percent_P.csv"))
  # 可视化
  tryCatch({
    # 转换为数据框用于ggplot
    ...
    p_cross <- ggplot(cross_df, aes(x = HCF_type_en, y = Percent, fill = pathway_cluster_en)) +
      ...
      labs(title = "Figure S1. Pathway Distribution Across HCF Types (P Cycle)",
    ...
    print(p_cross)
    # 保存PDF
    ggsave(file.path(RESULTS_DIR, "figure_S1_cross_tab_P.pdf"), 
           p_cross, width = 10, height = 7, dpi = 300)
    # 保存PNG
    ggsave(file.path(RESULTS_DIR, "figure_S1_cross_tab_P.png"), 
           p_cross, width = 10, height = 7, dpi = 300)
    cat(" ✅ Figure S1 saved: figure_S1_cross_tab_P.pdf and .png\n")
  }, error = function(e) {
    cat(" 可视化失败:", e$message, "\n")
  })
}
...
if (length(alpha_vars) > 0) {
  ...
  # 保存结果（英文版）
  write.csv(alpha_by_cluster, file.path(RESULTS_DIR, "Table2_P.csv"), row.names = FALSE)
  cat(" ✅ Table S2 saved: Table2_P.csv\n")
  # 统计检验
...
cat("补充分析3：四通路预测硬结局\n")
cat("========================================================\n")
# 根据L周期经验，hypertension_doctor可能不存在，先只分析糖尿病
outcome_vars <- c(
  "diabetes_doctor" = "Diabetes"
  # 如果有高血压变量，可以添加
)
# 检查是否有高血压变量
if("hypertension" %in% names(high_risk_data)) {
  outcome_vars <- c(outcome_vars, "hypertension" = "Hypertension")
} else if("hypertension_doctor" %in% names(high_risk_data)) {
  outcome_vars <- c(outcome_vars, "hypertension_doctor" = "Hypertension")
}
...
if (length(outcome_vars) > 0) {
  ...
  if (!is.null(all_results) && nrow(all_results) > 0) {
    write.csv(all_results, file.path(RESULTS_DIR, "table_S3_outcome_prediction_P.csv"), row.names = FALSE)
    cat("\n ✅ Table S3 saved: table_S3_outcome_prediction_P.csv\n")
    cat("\n【完全调整模型结果】\n")
...
if (nrow(behavior_results) > 0) {
  write.csv(behavior_results, file.path(RESULTS_DIR, "table_S4_behavior_effects_all_P.csv"), row.names = FALSE)
...
if ("risk_progression_score" %in% names(high_risk_data) &&
  ...
  if (nrow(roc_data) > 0 && require(pROC, quietly = TRUE)) {
    ...
    write.csv(auc_result, file.path(RESULTS_DIR, "table_S5_risk_auc_P.csv"), row.names = FALSE)
    cat("\n ✅ Table S5 saved: table_S5_risk_auc_P.csv\n")
    saveRDS(roc_obj, file.path(RESULTS_DIR, "roc_object_P.rds"))
    cat(" ✅ ROC object saved: roc_object_P.rds\n")
  }
}
# ============================================================================
...
if (file.exists(file.path(RESULTS_DIR, "table_S1_cross_tab_percent_P.csv")) &&
    file.exists(file.path(RESULTS_DIR, "Table2_P.csv"))) {
  cross_data <- read.csv(file.path(RESULTS_DIR, "table_S1_cross_tab_percent_P.csv"))
  alpha_data <- read.csv(file.path(RESULTS_DIR, "Table2_P.csv"))
...
cat("\n")
if (file.exists(file.path(RESULTS_DIR, "Table2_P.csv"))) {
  alpha_tab <- read.csv(file.path(RESULTS_DIR, "Table2_P.csv"))
  cat("【Alpha Factors × Pathway】\n")
  print(alpha_tab)
  cat("\n")
}
if (file.exists(file.path(RESULTS_DIR, "table_S3_outcome_prediction_P.csv"))) {
  outcome_tab <- read.csv(file.path(RESULTS_DIR, "table_S3_outcome_prediction_P.csv"))
  cat("【Outcome Prediction (Fully Adjusted Model)】\n")
  print(outcome_tab[outcome_tab$Model == "Fully adjusted",
                    c("Outcome", "Pathway", "OR", "CI_lower", "CI_upper", "P_value")])
  cat("\n")
}
if (file.exists(file.path(RESULTS_DIR, "table_S4_behavior_effects_all_P.csv"))) {
  behav_tab <- read.csv(file.path(RESULTS_DIR, "table_S4_behavior_effects_all_P.csv"))
  cat("【Behavior Effects】\n")
  print(behav_tab[, c("Behavior", "Beta", "P_value", "CI_lower", "CI_upper")])
  cat("\n")
}
if (file.exists(file.path(RESULTS_DIR, "table_S5_risk_auc_P.csv"))) {
  auc_tab <- read.csv(file.path(RESULTS_DIR, "table_S5_risk_auc_P.csv"))
  cat("【Risk Score AUC】\n")
  print(auc_tab)
  cat("\n")
}
...
code_save_path <- file.path(scripts_dir, "13_paper2_supplement_P.R")
cat("\n⚠️  请手动将当前脚本保存到以下位置：\n")
cat(sprintf("   %s\n\n", code_save_path))
...
# ============================================================================
cat("\n========================================================\n")
cat("✅ Paper 2 supplemental analysis complete (P Cycle)!\n")
cat("Completion time:", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n")
