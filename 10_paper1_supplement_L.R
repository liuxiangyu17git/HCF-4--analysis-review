#!/usr/bin/env Rscript
# ============================================================================
# 脚本: 10_paper1_supplement_L.R
# 描述: NHANES 2021-2023 (L周期) 论文1补充分析
# 
...
# 日志
log_file <- file.path(LOG_DIR, paste0("10_paper1_supp_L_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".log"))
...
write.csv(mediation_results, file.path(results_dir, "paper1_supp_table1_mediation.csv"), row.names = FALSE)
...
if(nrow(pa_results) > 0) {
  ...
  write.csv(pa_results_out, file.path(results_dir, "paper1_supp_table2_pa_effects.csv"), row.names = FALSE)
  ...
  p <- ggplot(plot_data, aes(x = HCF_type, y = Beta, fill = HCF_type)) +
    ...
    labs(title = "Figure S1. Physical Activity Effects on Depression by HCF Type",
         x = "", y = "β Coefficient (95% CI)") +
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1),
          legend.position = "none")
  # 同时保存PDF和PNG
  ggsave(file.path(results_dir, "paper1_figure_s1_pa_effects.pdf"), p, width = 8, height = 6)
  ggsave(file.path(results_dir, "paper1_figure_s1_pa_effects.png"), p, width = 8, height = 6, dpi = 300)
  cat(" ✅ Figure S1 saved: PDF and PNG formats\n")
}
# ============================================================================
# 7. 分析3：生物标志物梯度分析
# ============================================================================
...
if(nrow(biomarker_results) > 0) {
  ...
  write.csv(biomarker_results_out, file.path(results_dir, "paper1_supp_table3_biomarker.csv"), row.names = FALSE)
...
if(nrow(clinical_results) > 0) {
  ...
  write.csv(clinical_results_out, file.path(results_dir, "paper1_supp_table4_clinical.csv"), row.names = FALSE)
...
cat("9. 生成补充分析报告\n")
cat("========================================================\n")
report_file <- file.path(LOG_DIR, "10_paper1_supp_report_L.txt")
sink(report_file)
cat("论文1补充分析报告\n")
cat("==================\n\n")
...
cat("\n五、输出文件清单\n")
cat(" - paper1_supp_table1_mediation.csv\n")
cat(" - paper1_supp_table2_pa_effects.csv\n")
cat(" - paper1_supp_table3_biomarker.csv\n")
cat(" - paper1_supp_table4_clinical.csv\n")
cat(" - paper1_figure_s1_pa_effects.pdf\n")
cat(" - paper1_figure_s1_pa_effects.png\n")
sink()
cat("\n ✅ 补充分析报告已保存\n\n")
# ============================================================================
# 10. 保存会话信息
# ============================================================================
cat("10. 保存会话信息...\n")
session_info_path <- file.path(LOG_DIR, "10_session_info_L.txt")
sink(session_info_path)
cat("NHANES L周期论文1补充分析会话信息\n")
cat("==================================\n")
...
code_save_path <- file.path(scripts_dir, "10_paper1_supplement_L.R")
cat("\n⚠️  请手动将当前脚本保存到以下位置：\n")
cat(sprintf("   %s\n\n", code_save_path))
cat("   这是JAMA Psychiatry的明确要求：所有分析代码必须保存并公开。\n")
code_list_path <- file.path(LOG_DIR, "10_code_list_L.txt")
cat("脚本名称: 10_paper1_supplement_L.R\n", file = code_list_path)
