...
cat("描述: NHANES 2017-2020 (P周期) 论文2 - 四通路聚类分析\n")
...
write.csv(name_mapping, file.path(RESULTS_DIR, "cluster_naming_P.csv"), row.names = FALSE)
cat(" ✅ 聚类命名已保存\n\n")
# ============================================================================
...
p1 <- ggplot(plot_data, aes(x = dimension_en, y = value, fill = pathway_cluster_en)) +
  ...
  labs(title = "Figure 1. Four Psychophysiological Pathways (P Cycle)",
...
# 保存图表
ggsave(file.path(RESULTS_DIR, "Figure1_P.pdf"), p1, width = 10, height = 6)
ggsave(file.path(RESULTS_DIR, "Figure1_P.png"), p1, width = 10, height = 6, dpi = 300)
cat(" ✅ Figure 1. Four Psychophysiological Pathways (P Cycle) saved\n")
# ============================================================================
...
# 保存热图
pdf(file.path(RESULTS_DIR, "figure2_cluster_heatmap_P.pdf"), width = 14, height = 7)
pheatmap(
  heat_matrix_scaled,
  color = colorRampPalette(c("navy", "white", "firebrick3"))(50),
  main = "Figure 2. Pathway Cluster Characteristics Heatmap (P Cycle)",
  fontsize_row = 11,
  fontsize_col = 10,
  angle_col = 45,
...
dev.off()
cat(" ✅ Figure 2. Pathway Cluster Characteristics Heatmap (P Cycle) saved\n\n")
# ============================================================================
# 10. 聚类特征表（加权）- Table 1
# ============================================================================
cat("7. 生成聚类特征表...\n")
# 注意：P周期使用 WTINTPRP，不是 WTINT2YR
design <- svydesign(
  id = ~SDMVPSU,
  strata = ~SDMVSTRA,
  weights = ~WTINTPRP,
...
for (i in 1:n_clusters) {
  cluster_num <- i  # 直接用数字
  cluster_name_en <- cluster_names_levels[i]
...
# 转置使每个聚类为一列（如果需要）
characteristics_t <- as.data.frame(t(characteristics))
colnames(characteristics_t) <- characteristics_t[1,]
characteristics_t <- characteristics_t[-1,]
# 保存
write.csv(characteristics_t, file.path(RESULTS_DIR, "Table1_P.csv"), row.names = TRUE)
cat(" ✅ 聚类特征表已保存\n\n")
# ============================================================================
# 11. 四通路扩展分析
# ============================================================================
...
write.csv(risk_by_cluster, file.path(RESULTS_DIR, "risk_score_by_cluster_P.csv"), row.names = FALSE)
# 11.2 通路与临床结局的关联
...
write.csv(outcome_analysis, file.path(RESULTS_DIR, "outcome_by_cluster_P.csv"), row.names = FALSE)
cat(" ✅ 扩展分析完成\n\n")
# 在这里添加 save()
save(
  km_result,
  ...
  outcome_analysis,
  characteristics_t,
  file = file.path(RESULTS_DIR, "paper2_results_P.RData")
)
cat(" ✅ 聚类结果已保存: paper2_results_P.RData\n\n")
# ============================================================================
# 12. 保存所有结果
# ============================================================================
cat("9. 保存结果...\n")
save_vars <- c(
  "SEQN", "WTINTPRP", "SDMVPSU", "SDMVSTRA", "HCF_type",
...
saveRDS(high_risk_final, file.path(RESULTS_DIR, "paper2_analysis_data_P.rds"))
cat(" ✅ 分析数据已保存: paper2_analysis_data_P.rds\n")
# ============================================================================
# 13. 生成分析报告
# ============================================================================
cat("10. 生成分析报告...\n")
...
cat("\n四、Output Files\n")
cat("  📊 Figure 1: Figure1_P.pdf\n")
cat("  📊 Figure 2: figure2_cluster_heatmap_P.pdf\n")
cat("  📋 Table 1: Table1_P.csv\n")
cat("  📋 Table 2: cluster_naming_P.csv\n")
cat("  📋 Table 3: risk_score_by_cluster_P.csv\n")
cat("  📋 Table 4: outcome_by_cluster_P.csv\n")
cat("  💾 Data: paper2_analysis_data_P.rds\n")
cat("  💾 Results: paper2_results_P.RData\n")
sink()
cat(" ✅ 分析报告已保存\n\n")
# ============================================================================
# 14. 保存会话信息（期刊要求）
...
code_save_path <- file.path(scripts_dir, "12_paper2_pathway_analysis_P.R")
cat("\n⚠️  请手动将当前脚本保存到以下位置：\n")
cat(sprintf("   %s\n\n", code_save_path))
...
cat("\n========================================================\n")
cat("✅ Paper 2 analysis complete (P Cycle)!\n")
cat("Completion time:", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "\n")
cat("========================================================\n")
sink()
# 清理临时变量
